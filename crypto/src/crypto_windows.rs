...
mod custom_chain_policy_tests {
    ...
    impl std::fmt::Display for PolicyResult {
        fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            write!(
                formatter,
                "called={}, error=0x{:08X}, chain={}, element={}",
                self.called, self.error, self.chain, self.element
            )
        }
    }
    ...
    fn policy(
        policy: PCSTR,
        certificates: &[&NativeCertificate],
        context_error: u32,
        chain_error: u32,
        element_errors: &[u32],
    ) -> PolicyResult {
        assert_eq!(certificates.len(), element_errors.len());
        let mut elements = certificates
            .iter()
            .zip(element_errors)
            .map(|(certificate, error)| Crypto32::CERT_CHAIN_ELEMENT {
                cbSize: size_of::<Crypto32::CERT_CHAIN_ELEMENT>() as u32,
                pCertContext: certificate.as_ptr(),
                TrustStatus: Crypto32::CERT_TRUST_STATUS {
                    dwErrorStatus: *error,
                    ..Default::default()
                },
                ..Default::default()
            ...
            .collect::<Vec<_>>();
        let mut element_pointers = elements
            .iter_mut()
            .map(|element| element as *mut _)
            .collect::<Vec<_>>();
        let mut simple_chain = Crypto32::CERT_SIMPLE_CHAIN {
            cbSize: size_of::<Crypto32::CERT_SIMPLE_CHAIN>() as u32,
            TrustStatus: Crypto32::CERT_TRUST_STATUS {
                dwErrorStatus: chain_error,
                ..Default::default()
            },
            cElement: element_pointers.len() as u32,
            rgpElement: element_pointers.as_mut_ptr(),
            ..Default::default()
        };
        let mut simple_chains = [&mut simple_chain as *mut _];
        let chain_context = Crypto32::CERT_CHAIN_CONTEXT {
            cbSize: size_of::<Crypto32::CERT_CHAIN_CONTEXT>() as u32,
            TrustStatus: Crypto32::CERT_TRUST_STATUS {
                dwErrorStatus: context_error,
                ..Default::default()
            },
            cChain: simple_chains.len() as u32,
            rgpChain: simple_chains.as_mut_ptr(),
            ..Default::default()
        };
        let parameters = Crypto32::CERT_CHAIN_POLICY_PARA {
            cbSize: size_of::<Crypto32::CERT_CHAIN_POLICY_PARA>() as u32,
            ..Default::default()
        };
        let mut status = Crypto32::CERT_CHAIN_POLICY_STATUS {
            cbSize: size_of::<Crypto32::CERT_CHAIN_POLICY_STATUS>() as u32,
            ..Default::default()
        };
        let called = unsafe {
            Crypto32::CertVerifyCertificateChainPolicy(
                policy,
                &chain_context,
                &parameters,
                &mut status,
        ...
        PolicyResult {
            ...
            chain: status.lChainIndex,
            element: status.lElementIndex,
        }
    }

    fn certificates() -> (NativeCertificate, NativeCertificate, NativeCertificate) {
        ...
        (
            NativeCertificate::from_der(root.der()).expect("root context should open"),
            NativeCertificate::from_der(intermediate.der())
                .expect("intermediate context should open"),
            NativeCertificate::from_der(leaf.der()).expect("leaf context should open"),
        )
    }

    #[test]
    fn base_policy_custom_chain_matrix() {
        let (root, intermediate, leaf) = certificates();
        let valid = [&leaf, &intermediate, &root];
        let wrong_issuer = [&leaf, &leaf, &root];
        let cases = [
            ("zero-status valid path", valid.as_slice(), 0, 0, [0, 0, 0]),
            (
                "zero-status wrong issuer",
                wrong_issuer.as_slice(),
                0,
                0,
                [0, 0, 0],
            ),
            (
                ...
                valid.as_slice(),
                Crypto32::CERT_TRUST_IS_NOT_SIGNATURE_VALID,
                0,
                [0, 0, 0],
            ),
            (
                "simple-chain signature error",
                valid.as_slice(),
                0,
                Crypto32::CERT_TRUST_IS_NOT_SIGNATURE_VALID,
                [0, 0, 0],
            ...
            (
                "issuer-element signature error",
                valid.as_slice(),
                0,
                0,
                [0, Crypto32::CERT_TRUST_IS_NOT_SIGNATURE_VALID, 0],
            ),
            (
                "context time error",
                valid.as_slice(),
                Crypto32::CERT_TRUST_IS_NOT_TIME_VALID,
                0,
                [0, 0, 0],
            ),
            (
                "context untrusted root",
                valid.as_slice(),
                Crypto32::CERT_TRUST_IS_UNTRUSTED_ROOT,
                0,
                [0, 0, 0],
            ),
        ];
        let results = cases
            .iter()
            .map(
                |(name, chain, context_error, chain_error, element_errors)| {
                    format!(
                        "{name}: {}",
                        policy(
                            Crypto32::CERT_CHAIN_POLICY_BASE,
                            chain,
                            *context_error,
                            *chain_error,
                            element_errors,
        ...
        panic!("base policy custom-chain matrix:\n{}", results.join("\n"));
    }

    #[test]
    fn basic_constraints_custom_chain_matrix() {
        let (root, intermediate, leaf) = certificates();
        let valid = [&leaf, &intermediate, &root];
        let leaf_as_issuer = [&leaf, &leaf, &root];
        let extra_ca = [&leaf, &intermediate, &intermediate, &root];
        let cases = [
            ("valid path", valid.as_slice()),
            ("leaf used as issuer", leaf_as_issuer.as_slice()),
            ("extra CA in path", extra_ca.as_slice()),
        ];
        let results = cases
            .iter()
            .map(|(name, chain)| {
                format!(
                    "{name}: {}",
                    policy(
                        Crypto32::CERT_CHAIN_POLICY_BASIC_CONSTRAINTS,
                        chain,
                        0,
                        0,
                        &vec![0; chain.len()],
        ...
        panic!(
            "basic constraints custom-chain matrix:\n{}",
            results.join("\n")
        );
    }
}

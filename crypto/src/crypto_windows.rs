        ...
        unsafe {
            Crypto32::CertGetCertificateChain(
                ...
                Crypto32::CERT_CHAIN_CACHE_ONLY_URL_RETRIEVAL
                    ...
                    | Crypto32::CERT_CHAIN_ONLY_ADDITIONAL_AND_AUTH_ROOT
...
mod isolated_chain_tests {
    ...
    struct InstalledCertificate(*const Crypto32::CERT_CONTEXT);

    impl Drop for InstalledCertificate {
        fn drop(&mut self) {
            unsafe {
                Crypto32::CertDeleteCertificateFromStore(self.0)
                    .expect("ambient test certificate should be removed");
            }
        }
    }
    ...
    fn chain_status(
        trusted: &Certificate,
        leaf: &Certificate,
        additional: Option<Crypto32::HCERTSTORE>,
        flags: u32,
    ) -> Result<u32> {
        let roots = CertStore::new()?;
        roots.add(trusted)?;
        ...
        let leaf = NativeCertificate::from_der(leaf.der())?;
        let mut raw_chain = std::ptr::null_mut();
        unsafe {
            Crypto32::CertGetCertificateChain(
                Some(*engine),
    ...
    #[test]
    fn only_additional_and_auth_root_excludes_ambient_ca_store() {
        ...
        let leaf = Crypto::from_pem(MILAN_VCEK).expect("leaf should parse");
        let ambient_store = CertStore(
            unsafe { Crypto32::CertOpenSystemStoreW(None, windows::core::w!("CA")) }
                .expect("CurrentUser CA store should open"),
        );
        let mut installed = std::ptr::null_mut();
        unsafe {
            Crypto32::CertAddEncodedCertificateToStore(
                Some(ambient_store.0),
                Crypto32::X509_ASN_ENCODING,
                intermediate.der(),
        ...
        .expect("intermediate should be installed in CurrentUser CA");
        NonNull::new(installed).expect("installed certificate should be returned");
        let _installed = InstalledCertificate(installed);

        let base_flags = Crypto32::CERT_CHAIN_CACHE_ONLY_URL_RETRIEVAL
            | Crypto32::CERT_CHAIN_DISABLE_AIA
            | Crypto32::CERT_CHAIN_DISABLE_AUTH_ROOT_AUTO_UPDATE;
        assert_eq!(
            chain_status(&root, &leaf, None, base_flags).expect("ambient chain should build"),
            Crypto32::CERT_TRUST_NO_ERROR,
            "baseline should resolve the intermediate from CurrentUser CA"
        );
        assert_eq!(
            chain_status(
                &root,
                &leaf,
                None,
                base_flags | Crypto32::CERT_CHAIN_ONLY_ADDITIONAL_AND_AUTH_ROOT,
            )
            .expect("isolated partial chain should build"),
            Crypto32::CERT_TRUST_IS_PARTIAL_CHAIN,
            "isolated building should exclude CurrentUser CA"
        );

        let supplied = CertStore::new().expect("supplied store should open");
        supplied.add(&root).expect("root should be supplied");
        supplied
            .add(&intermediate)
            .expect("intermediate should be supplied");
        assert_eq!(
            chain_status(
                &root,
                &leaf,
            ...
            .expect("isolated supplied chain should build"),
            Crypto32::CERT_TRUST_NO_ERROR,
            "isolated building should use the explicitly supplied chain"
        );

        let auth_roots = CertStore(
            unsafe { Crypto32::CertOpenSystemStoreW(None, windows::core::w!("AuthRoot")) }
                .expect("CurrentUser AuthRoot store should open"),
        );
        let auth_root = unsafe { Crypto32::CertEnumCertificatesInStore(auth_roots.0, None) };
        let auth_root =
            NonNull::new(auth_root).expect("CurrentUser AuthRoot store should not be empty");
        let auth_root = NativeCertificate(auth_root);
        let auth_root = Certificate::from_der(
            auth_root
                .der()
                .expect("AuthRoot certificate DER should be readable"),
        )
        .expect("AuthRoot certificate should parse");
        let supplied_auth_root = CertStore::new().expect("AuthRoot supplied store should open");
        supplied_auth_root
            .add(&auth_root)
            .expect("AuthRoot certificate should be supplied");
        assert_ne!(
            chain_status(
                &root,
                &auth_root,
                Some(supplied_auth_root.0),
                base_flags | Crypto32::CERT_CHAIN_ONLY_ADDITIONAL_AND_AUTH_ROOT,
            )
            .expect("AuthRoot chain should build"),
            Crypto32::CERT_TRUST_NO_ERROR,
            "AuthRoot must not bypass the configured exclusive root"
        );
    }

    #[test]
    fn select_certificate_chains_uses_ambient_intermediates() {
        let root = Crypto::from_pem(MILAN_ARK).expect("root should parse");
        let intermediate = Crypto::from_pem(MILAN_ASK).expect("intermediate should parse");
        let leaf = Crypto::from_pem(MILAN_VCEK).expect("leaf should parse");
        ...
        let mut engine_config = Crypto32::CERT_CHAIN_ENGINE_CONFIG {
            cbSize: size_of::<Crypto32::CERT_CHAIN_ENGINE_CONFIG>() as u32,
            hExclusiveRoot: roots.0,
            hExclusiveTrustedPeople: roots.0,
            dwExclusiveFlags: Crypto32::CERT_CHAIN_EXCLUSIVE_ENABLE_CA_FLAG,
        ...
        let engine = into_owned(|handle| unsafe {
            Crypto32::CertCreateCertificateChainEngine(&mut engine_config, handle)
        })
        .expect("chain engine should be created");
        let leaf_store = CertStore::new().expect("leaf store should open");
        leaf_store.add(&leaf).expect("leaf should be added");
        let mut chain_parameters = Crypto32::CERT_CHAIN_PARA {
            cbSize: size_of::<Crypto32::CERT_CHAIN_PARA>() as u32,
            ..Default::default()
        };
        let selection_parameters = Crypto32::CERT_SELECT_CHAIN_PARA {
            hChainEngine: *engine,
            pChainPara: &mut chain_parameters,
            dwFlags: Crypto32::CERT_CHAIN_CACHE_ONLY_URL_RETRIEVAL,
            ..Default::default()
        };
        let mut count = 0;
        let mut selections = std::ptr::null_mut();
        unsafe {
            Crypto32::CertSelectCertificateChains(
                None,
                Crypto32::CERT_SELECT_TRUSTED_ROOT,
                Some(&selection_parameters),
                None,
                leaf_store.0,
                &mut count,
                &mut selections,
        ...
        .expect("certificate chains should be selected");
        assert_eq!(
            count, 1,
            "leaf-only selection store should yield its trusted chain"
        );
        let selected = unsafe { std::slice::from_raw_parts(selections, count as usize) };
        let chain = unsafe { selected[0].as_ref() }.expect("selected chain should not be null");
        let simple_chains =
            unsafe { native_slice(chain.rgpChain, chain.cChain, &selection_parameters) }
                .expect("simple chains should be readable");
        let simple = unsafe { simple_chains[0].as_ref() }.expect("simple chain should not be null");
        assert_eq!(
            simple.cElement, 3,
            "selected chain should include ambient ASK and exclusive ARK"
        );
        for chain in selected {
            unsafe { Crypto32::CertFreeCertificateChain(*chain) };
        }
        unsafe {
            Crypto32::CertFreeCertificateChainList(
                selections.cast::<*const Crypto32::CERT_CHAIN_CONTEXT>(),
            )
        };
    }
}

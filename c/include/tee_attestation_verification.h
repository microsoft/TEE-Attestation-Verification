...
struct AttestationReport {
    ...
    struct TcbVersionRaw  platform_version;  /* 0x038 */
    ...
    struct TcbVersionRaw  reported_tcb;      /* 0x180 */
    ...
    struct TcbVersionRaw  committed_tcb;     /* 0x1E0 */
    ...
    struct TcbVersionRaw  launch_tcb;        /* 0x1F0 */
    uint8_t               reserved4[168];    /* 0x1F8 */
    struct SnpSignature   signature;         /* 0x2A0 */
};
 ...
 *         AttestationReport.  The pointer borrows from report_ptr — the
 *         caller must keep the report buffer alive while using it.
 *         On failure, returns NULL and sets *err_out.
 */
const struct AttestationReport *tav_verify_attestation(
...
/** Free an error returned by tav_verify_attestation().

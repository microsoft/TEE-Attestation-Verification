...
/*
 * Parse CBOR bytes into an owned value tree.
 *
 * out_value must point to a writable handle slot that contains NULL on entry.
 * If out_value is NULL, or *out_value is non-NULL, this returns
 * TAV_ERROR_INVALID_ARGUMENT and does not overwrite an existing handle.
 */
